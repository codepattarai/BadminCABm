...
class AppState extends ChangeNotifier {
  ...
  void _onTimerComplete() async {
    ...
    _isInBreak = true; // NEW: Mark as in break period
    notifyListeners();
    
    // FIXED: Play sound properly with multiple attempts
    try {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        await HapticFeedback.heavyImpact(); // Stronger vibration
    ...
    // FIXED: Wait for break duration BEFORE reassigning
    await Future.delayed(Duration(seconds: _breakDuration));
    
    // After break, reassign courts
    if (_selectedPlayers.length >= 4) {
      _currentRound++;
      _timeRemaining = _matchDuration * 60;
      ...
      try {
        autoAssignCourts();
        _isInBreak = false; // NEW: Break is over
                    ...
                    children: [
                      ...
                      ...players.map((playerName) {
                        ...
                        return Padding(
                          ...
                          child: Container(
                            width: double.infinity, // Full width chips
