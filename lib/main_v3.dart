...
// All issues resolved: sound, pairing, rest rotation, sizing
...
class AppState extends ChangeNotifier {
  ...
  void autoAssignCourts() {
    ...
    // Build units (pairs and singles) maintaining selection order
    final units = _buildUnitsInOrder();
    
    // Calculate how many players will rest
    final totalPlayers = _selectedPlayers.length;
    final numRestingPlayers = max(0, totalPlayers - totalPlayingSlots);
    
    // Determine resting units using fair rotation
  ...
  List<List<String>> _buildUnitsInOrder() {
    // Group players by pair number, maintaining selection order
    ...
    for (final playerId in _selectedPlayers) {
      ...
      final pairNum = player.pairNum;  // Store in local variable
		if (pairNum != null) {
		  if (!pairGroups.containsKey(pairNum)) {
			pairGroups[pairNum] = [];  // Now this works!
		  }
    ...
    // Build units maintaining order
    final units = <List<String>>[];
    
    // Add valid pairs (exactly 2 players)
    for (final pairNum in pairGroups.keys) {
      final pair = pairGroups[pairNum]!;
      if (pair.length == 2) {
        units.add(pair);
      } else {
        // Invalid pair, treat as singles
        singles.addAll(pair);
      }
    }
  ...
  void _onTimerComplete() async {
    ...
    // Play multiple beeps for better audibility
    for (int i = 0; i < 3; i++) {
    ...
    if (_selectedPlayers.length >= 4) {
      ...
      try {
        ...
        // Now wait for break duration for players to check assignments
        // Timer is already running, this is just for display purposes
      } catch (e) {
        // If auto-assign fails, just notify
        notifyListeners();
      }
    } else {
      notifyListeners();
...
class DashboardScreen extends StatelessWidget {
            ...
            onPressed: () {
              try {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Courts assigned & timer started!'),
                        ...
                        return Padding(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: isPaired ? Colors.orange[100] : Colors.blue[100],
  ...
  bool _isPaired(String playerName, AppState appState) {
    // Check if player has a pair partner
